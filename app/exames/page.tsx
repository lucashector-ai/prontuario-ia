...
export default function Exames() {
  ...
  const statusCor = (s: string) => {
    ...
    return { bg: '#f0fdf4', border: '#bbf7d0', text: '#166534', badge: '#16a34a', badgeBg: '#f0fdf4' }
  }
        ...
        <div style={{ flex: 1, overflow: 'auto', padding: 28, display: 'grid', gridTemplateColumns: analise ? '1fr 1fr' : '1fr', gap: 20 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div
              ...
              style={{
                background: 'white', border: `2px dashed ${preview ? '#bbf7d0' : '#e5e7eb'}`,
            ...
            <button onClick={handleAnalisar} disabled={!imagem || analisando} style={{
              ...
              background: !imagem ? '#f3f4f6' : analisando ? '#bbf7d0' : '#16a34a',
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14, overflow: 'auto' }}>
              ...
              <div style={{ background: 'white', border: '1px solid #e5e7eb', borderRadius: 12, padding: '16px 20px' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, marginBottom: 10 }}>
                  ...
                  <span style={{ fontSize: 11, color: '#16a34a', background: '#f0fdf4', border: '1px solid #bbf7d0', padding: '3px 10px', borderRadius: 20, fontWeight: 600, flexShrink: 0 }}>
                    ✓ Analisado pela IA
                  </span>
                </div>
                <p style={{ fontSize: 13, color: '#374151', margin: 0, lineHeight: 1.6, background: '#f9fafb', borderRadius: 8, padding: '10px 12px' }}>{analise.resumo}</p>
              </div>
              ...
              {analise.valores?.length > 0 && (
                <div style={{ background: 'white', border: '1px solid #e5e7eb', borderRadius: 12, overflow: 'hidden' }}>
                  ...
                  {analise.valores.map((v: any, i: number) => {
                    ...
                    return (
                      <div key={i} style={{ padding: '12px 16px', borderBottom: i < analise.valores.length - 1 ? '1px solid #f9fafb' : 'none', display: 'flex', alignItems: 'flex-start', gap: 12 }}>
                        <div style={{ flex: 1 }}>
              ...
              {analise.recomendacoes?.length > 0 && (
                <div style={{ background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: 12, padding: '16px 20px' }}>
                  <p style={{ fontSize: 11, fontWeight: 700, color: '#16a34a', margin: '0 0 10px', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Recomendações</p>
                  {analise.recomendacoes.map((r: string, i: number) => (
                    <div key={i} style={{ display: 'flex', gap: 10, marginBottom: 6 }}>
                      ...
                      <p style={{ fontSize: 13, color: '#166534', margin: 0, lineHeight: 1.6 }}>{r}</p>
                    </div>
                  ))}
                </div>
              )}
