...
export default function Teleconsulta() {
  ...
  const criar = async (e: React.FormEvent) => {
    ...
    const d = await r.json()
    if (d.teleconsulta) {
      setModal(false); setForm({ paciente_id: '', titulo: '' })
      carregar(medico.id)
    }
    setCriando(false)
  }
  ...
  const enviarWhatsApp = async (consulta: any) => {
    ...
    const msg = 'Ola! O Dr(a). ' + medico.nome + ' te convidou para uma teleconsulta. Acesse o link abaixo para entrar na sala de video:\n\n' + link + '\n\nNao precisa instalar nada, funciona direto no navegador.'
    if (consulta.pacientes?.telefone) {
      ...
      alert('Paciente sem telefone cadastrado. Mensagem copiada!')
    }
  }

  const statusLabel = (s: string) => ({ aguardando: { txt: 'Aguardando', cor: '#f59e0b', bg: '#fef3c7' }, em_andamento: { txt: 'Em andamento', cor: '#16a34a', bg: '#dcfce7' }, encerrada: { txt: 'Encerrada', cor: '#6b7280', bg: '#f3f4f6' } }[s] || { txt: s, cor: '#6b7280', bg: '#f3f4f6' })
        ...
        <div style={{ maxWidth: 860 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
            <div>
              ...
              <p style={{ fontSize: 13, color: '#6b7280', margin: 0 }}>Consultas de video sem app — o paciente entra pelo link</p>
            </div>
          ...
          <div style={{ background: 'white', border: '1px solid #e5e7eb', borderRadius: 14, padding: '16px 20px', marginBottom: 24, display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 16 }}>
            {[
              ...
              { n: '4', txt: 'Consulta em video', sub: 'peer-to-peer' },
            ].map(s => (
          ...
          {consultas.length === 0 ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {consultas.map(c => {
                ...
                const link = typeof window !== 'undefined' ? window.location.origin + '/sala/' + c.sala_id : ''
                return (
                  <div key={c.id} style={{ background: 'white', border: '1px solid #e5e7eb', borderRadius: 12, padding: '14px 18px', display: 'flex', alignItems: 'center', gap: 14 }}>
                    ...
                    {c.status !== 'encerrada' && (
                      <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
                        <button onClick={() => copiarLink(c.sala_id)} title="Copiar link" style={{ padding: '7px 12px', background: 'white', border: '1px solid #e5e7eb', borderRadius: 7, fontSize: 11, color: '#6b7280', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 5 }}>
                          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 01-2-2V4a2 2 0 012-2h9a2 2 0 012 2v1"/></svg>
                          Copiar link
                        </button>
                        <button onClick={() => enviarWhatsApp(c)} title="Enviar por WhatsApp" style={{ padding: '7px 12px', background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: 7, fontSize: 11, color: '#16a34a', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 5, fontWeight: 600 }}>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 999 }}>
          <div style={{ background: 'white', borderRadius: 16, padding: '24px', width: 420, boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }}>
            ...
            <form onSubmit={criar} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div>
                <label style={{ fontSize: 12, fontWeight: 600, color: '#374151', display: 'block', marginBottom: 5 }}>Titulo da consulta</label>
                <input value={form.titulo} onChange={e => setForm(f => ({ ...f, titulo: e.target.value }))} style={{ width: '100%', padding: '9px 12px', fontSize: 13, borderRadius: 8, border: '1.5px solid #e5e7eb' }} placeholder="Ex: Consulta de retorno, Avaliação inicial..."/>
              </div>
