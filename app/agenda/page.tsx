                ...
                {Array.from({ length: (HORA_FIM - HORA_INI) * 4 }, (_, i) => {
                  ...
                  return (
                    <div key={i}
                      ...
                      style={{ height: SLOT_PX, borderTop: isHoraCheia ? '1px solid #f3f4f6' : (isMeia ? '1px dashed #FAFAFA' : 'none'), cursor: 'pointer', transition: 'background 0.1s' }}
              ...
              {Array.from({ length: (HORA_FIM - HORA_INI) * 4 }, (_, i) => {
                ...
                return (
                  <div key={i}
                    ...
                    style={{ height: SLOT_PX, borderTop: isHoraCheia ? '1px solid #f3f4f6' : (isMeia ? '1px dashed #FAFAFA' : 'none'), cursor: 'pointer' }}
                    onMouseOver={e => { e.currentTarget.style.background = 'rgba(96,67,193,0.04)' }}
                    onMouseOut={e => { e.currentTarget.style.background = 'transparent' }}/>
                )
              })}
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100, padding: 24 }}
          ...
          <div style={{ background: 'white', borderRadius: 16, width: '100%', maxWidth: 480, overflow: 'hidden' }}>
            ...
            <form onSubmit={salvar} style={{ padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              {!modal.ag && (
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 14px', background: comVideo ? '#f3f0fd' : '#FAFAFA', borderRadius: 10, border: '1px solid ' + (comVideo ? '#d4c9f7' : '#e5e7eb') }}>
