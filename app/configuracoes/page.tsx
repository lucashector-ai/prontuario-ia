    ...
    <div style={{ display: 'flex', height: '100vh', background: '#f9fafb', overflow: 'hidden' }}>
      ...
      <main style={{ flex: 1, overflow: 'auto', padding: 32 }}>
        <div style={{ maxWidth: 680 }}>
          ...
          {/* Status atual */}
          {config && (
            <div style={{ background: '#f0fdf4', border: '1px solid #d4c9f7', borderRadius: 12, padding: '16px 20px', marginBottom: 24, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <div style={{ background: 'white', border: '1px solid #e5e7eb', borderRadius: 14, overflow: 'hidden', marginBottom: 24 }}>
            ...
            <form onSubmit={salvar} style={{ padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              {msg && (
                <div style={{ background: msg.tipo === 'ok' ? '#f0fdf4' : '#fef2f2', border: '1px solid ' + (msg.tipo === 'ok' ? '#d4c9f7' : '#fecaca'), borderRadius: 9, padding: '10px 14px' }}>
                  <p style={{ fontSize: 13, color: msg.tipo === 'ok' ? '#6043C1' : '#dc2626', margin: 0 }}>{msg.texto}</p>
                </div>
              )}
              <button type="submit" disabled={salvando} style={{ padding: '11px', borderRadius: 9, border: 'none', background: salvando ? '#86efac' : '#6043C1', color: 'white', fontSize: 14, fontWeight: 700, cursor: 'pointer' }}>
