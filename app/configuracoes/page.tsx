    ...
    <div style={{ display: 'flex', height: '100vh', background: '#EAECEF', overflow: 'hidden' }}>
      <main style={{ flex: 1, overflow: 'auto', padding: 32 }}>
        <div style={{ maxWidth: 680 }}>
          ...
          {/* Status atual */}
          {config && (
            <div style={{ background: '#EAECEF', border: '1px solid #d4c9f7', borderRadius: 12, padding: '16px 20px', marginBottom: 24, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                <div>
                  <p style={{ fontSize: 13, fontWeight: 700, color: '#6043C1', margin: 0 }}>WhatsApp conectado</p>
                  <p style={{ fontSize: 12, color: '#6043C1', margin: 0 }}>{config.phone_number || config.phone_number_id} · {config.nome_exibicao}</p>
                </div>
              </div>
          ...
          <div style={{ background: 'white', borderRadius: 14, overflow: 'hidden', marginBottom: 24 }}>
            ...
            <form onSubmit={salvar} style={{ padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              {msg && (
                <div style={{ background: msg.tipo === 'ok' ? '#EAECEF' : '#fef2f2', border: '1px solid ' + (msg.tipo === 'ok' ? '#d4c9f7' : '#fecaca'), borderRadius: 9, padding: '10px 14px' }}>
                  <p style={{ fontSize: 13, color: msg.tipo === 'ok' ? '#6043C1' : '#dc2626', margin: 0 }}>{msg.texto}</p>
                </div>
              )}
              <button type="submit" disabled={salvando} style={{ padding: '11px', borderRadius: 9, border: 'none', background: salvando ? '#b9a9ef' : '#6043C1', color: 'white', fontSize: 14, fontWeight: 700, cursor: 'pointer' }}>
            ...
            <div style={{ padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 14 }}>
              {[
                ...
                <div key={s.n} style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
                  <div style={{ width: 22, height: 22, borderRadius: '50%', background: '#6043C1', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 800, color: 'white', flexShrink: 0, marginTop: 1 }}>{s.n}</div>
                ...
                {[
                  ...
                  <div key={item.label}>
                    ...
                    <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                      <code style={{ flex: 1, padding: '8px 12px', background: '#EAECEF', borderRadius: 7, fontSize: 12, color: '#374151', fontFamily: 'monospace' }}>{item.valor}</code>
