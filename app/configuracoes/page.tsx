    ...
    <div style={{ display: 'flex', height: '100vh', background: tokens.bg.hover, overflow: 'hidden' }}>
      <main style={{ flex: 1, overflow: 'auto', padding: 32 }}>
        <div style={{ maxWidth: 680 }}>
          ...
          {/* Status atual */}
          {config && (
            <div style={{ background: tokens.bg.hover, border: `1px solid ${tokens.brand.primaryAccent}`, borderRadius: 12, padding: '16px 20px', marginBottom: 24, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                <div>
                  <p style={{ fontSize: 13, fontWeight: 700, color: tokens.brand.primary, margin: 0 }}>WhatsApp conectado</p>
                  <p style={{ fontSize: 12, color: tokens.brand.primary, margin: 0 }}>{config.phone_number || config.phone_number_id} · {config.nome_exibicao}</p>
                </div>
              </div>
              <button onClick={remover} disabled={removendo} style={{ fontSize: 12, color: tokens.status.danger, background: tokens.status.dangerBg, border: `1px solid ${tokens.status.dangerLight}`, padding: '6px 14px', borderRadius: 7, cursor: 'pointer', fontWeight: 600 }}>
          ...
          <div style={{ background: 'white', borderRadius: 14, overflow: 'hidden', marginBottom: 24 }}>
            ...
            <form onSubmit={salvar} style={{ padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div>
                <label style={{ fontSize: 12, fontWeight: 600, color: tokens.text.strong, display: 'block', marginBottom: 6 }}>Phone Number ID <span style={{ color: tokens.status.danger }}>*</span></label>
              ...
              {msg && (
                <div style={{ background: msg.tipo === 'ok' ? tokens.bg.hover : tokens.status.dangerBg, border: '1px solid ' + (msg.tipo === 'ok' ? tokens.brand.primaryAccent : tokens.status.dangerLight), borderRadius: 9, padding: '10px 14px' }}>
                  <p style={{ fontSize: 13, color: msg.tipo === 'ok' ? tokens.brand.primary : tokens.status.danger, margin: 0 }}>{msg.texto}</p>
                </div>
              )}
              <button type="submit" disabled={salvando} style={{ padding: '11px', borderRadius: 9, border: 'none', background: salvando ? tokens.brand.primaryAccent : tokens.brand.primary, color: 'white', fontSize: 14, fontWeight: 700, cursor: 'pointer' }}>
          ...
          <div style={{ background: 'white', borderRadius: 14, overflow: 'hidden' }}>
            <div style={{ padding: '18px 24px', borderBottom: 'none' }}>
              <h2 style={{ fontSize: 14, fontWeight: 700, color: tokens.text.primary, margin: 0 }}>Configurar webhook no Meta</h2>
              <p style={{ fontSize: 12, color: tokens.text.secondary, margin: '4px 0 0' }}>Configure uma vez e todas as mensagens chegam automaticamente</p>
            </div>
            <div style={{ padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 14 }}>
              {[
                ...
                <div key={s.n} style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
                  <div style={{ width: 22, height: 22, borderRadius: '50%', background: tokens.brand.primary, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 800, color: 'white', flexShrink: 0, marginTop: 1 }}>{s.n}</div>
                  <p style={{ fontSize: 13, color: tokens.text.strong, margin: 0, lineHeight: 1.5 }}>{s.txt}</p>
                </div>
              ))}
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginTop: 4 }}>
                {[
                  ...
                  <div key={item.label}>
                    <p style={{ fontSize: 11, fontWeight: 600, color: tokens.text.secondary, margin: '0 0 4px', textTransform: 'uppercase', letterSpacing: '0.06em' }}>{item.label}</p>
                    <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                      <code style={{ flex: 1, padding: '8px 12px', background: tokens.bg.hover, borderRadius: 7, fontSize: 12, color: tokens.text.strong, fontFamily: 'monospace' }}>{item.valor}</code>
                      <button onClick={() => navigator.clipboard.writeText(item.valor)} style={{ padding: '7px 12px', background: 'white', borderRadius: 7, fontSize: 11, color: tokens.text.secondary, cursor: 'pointer', fontWeight: 600, flexShrink: 0 }}>Copiar</button>
                    </div>
                  </div>
                ))}
              </div>
