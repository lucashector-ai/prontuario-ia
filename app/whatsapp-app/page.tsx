...
export default function WhatsAppApp() {
  ...
  const md = (t:string) => t.replace(/\*\*(.*?)\*\*/g,'<strong>$1</strong>').replace(/\*(.*?)\*/g,'<em>$1</em>').replace(/---+/g,'<hr style="border:none;border-top:1px solid rgba(0,0,0,0.1);margin:4px 0"/>').split('\n').join('<br/>')
  ...
  useEffect(()=>{
    if(!medico) return
    carregarConversas()
    const ch=supabase.channel('wapp')
  ...
  useEffect(()=>{endRef.current?.scrollIntoView({behavior:'instant'})},[mensagens])

  const carregarConversas = useCallback(async()=>{
    if(!medico) return
    const {data}=await supabase.from('whatsapp_conversas').select('*,whatsapp_mensagens(conteudo,criado_em,tipo,lida)').eq('medico_id',medico.id).order('ultimo_contato',{ascending:false})
    if(!data) return
    setConversas(data.map((c:any)=>({...c,
  ...
  const enviar = async()=>{
    ...
    const {data:nova}=await supabase.from('whatsapp_mensagens').insert({conversa_id:ativa.id,tipo:'enviada',conteudo:texto,metadata:{manual:true,remetente:medico?.nome}}).select().single()
    ...
    <div style={{display:'flex',height:'100vh',width:'100vw',background:'#f0f2f5',overflow:'hidden',fontFamily:'-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif'}}>
      <style>{`
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .ibtn:hover{background:rgba(0,0,0,0.05)!important;border-radius:50%}
        .btn-resposta:hover{background:#f0fdf4!important}
      ...
      <div style={{width:56,background:'#f0f2f5',borderRight:'1px solid #d1d7db',display:'flex',flexDirection:'column',alignItems:'center',padding:'12px 0',flexShrink:0}}>
        ...
        {[
          {icon:<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M21 15a2 2 0 01-2 2H7l-4 4V5a2 2 0 012-2h14a2 2 0 012 2z"/></svg>,label:'Chats',active:true},
          {icon:<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>,label:'Status'},
          {icon:<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07A19.5 19.5 0 013.95 9.5a19.79 19.79 0 01-3.07-8.67A2 2 0 012.88 2h3a2 2 0 012 1.72c.127.96.361 1.903.7 2.81a2 2 0 01-.45 2.11L7.09 9.91a16 16 0 006 6z"/></svg>,label:'Canais'},
          {icon:<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 00-3-3.87"/><path d="M16 3.13a4 4 0 010 7.75"/></svg>,label:'Comunidades'},
        ].map(item=>(
          <div key={item.label} title={item.label} className="ibtn" style={{width:40,height:40,display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer',marginBottom:4,color:item.active?'#00a884':'#54656f'}}>
            {item.icon}
          </div>
        ))}
        <div style={{flex:1}}/>
      ...
      <div style={{width:390,background:'#ffffff',borderRight:'1px solid #d1d7db',display:'flex',flexDirection:'column',flexShrink:0}}>
        ...
        <div style={{padding:'13px 16px 10px',display:'flex',alignItems:'center',justifyContent:'space-between',background:'#f0f2f5'}}>
          <h1 style={{fontSize:20,fontWeight:600,color:'#111827'}}>WhatsApp</h1>
          <div style={{display:'flex',gap:2}}>
            <button onClick={()=>setNovaConversa(true)} className="ibtn" title="Nova conversa" style={{width:36,height:36,border:'none',background:'none',color:'#54656f',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center'}}>
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 013 3L7 19l-4 1 1-4L16.5 3.5z"/></svg>
            </button>
            <button className="ibtn" style={{width:36,height:36,border:'none',background:'none',color:'#54656f',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center'}}>
              <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><circle cx="12" cy="4" r="1.8"/><circle cx="12" cy="12" r="1.8"/><circle cx="12" cy="20" r="1.8"/></svg>
            </button>
          </div>
        </div>
        ...
        <div style={{display:'flex',gap:6,padding:'4px 12px 10px',overflowX:'auto'}}>
          {[{id:'todas',label:'Tudo'},{id:'nao_lidas',label:`Não lidas${total>0?\` ${total}`:''}`},{id:'ia',label:'Sofia IA'},{id:'humano',label:'Humano'}].map(f=>(
            <button key={f.id} onClick={()=>setFiltro(f.id as any)} style={{padding:'5px 14px',fontSize:13,fontWeight:500,borderRadius:20,border:'none',background:filtro===f.id?'#d1f4cc':'#f0f2f5',color:filtro===f.id?'#166534':'#54656f',cursor:'pointer',whiteSpace:'nowrap' as const,flexShrink:0,transition:'all 0.15s'}}>
              {f.label}
            </button>
          ))}
        </div>
        ...
        <div style={{flex:1,overflowY:'auto'}} onClick={()=>setMenuConversa(null)}>
          {filtradas.length===0?(
            <div style={{padding:'32px 16px',textAlign:'center'}}><p style={{fontSize:13,color:'#667781'}}>Nenhuma conversa</p></div>
          ):filtradas.map(cv=>(
            <div key={cv.id} className={`cv${ativa?.id===cv.id?' sel':''}`}
              ...
              <div style={{display:'flex',gap:12,alignItems:'center'}}>
                {cv.foto_url?(
                  ...
                  <div style={{width:49,height:49,borderRadius:'50%',background:'#dfe5e7',display:'flex',alignItems:'center',justifyContent:'center',fontSize:18,fontWeight:400,color:'#54656f',flexShrink:0}}>
                    {ini(nomeCv(cv))}
                  </div>
                )}
                <div style={{flex:1,minWidth:0}}>
                  ...
                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                    <p style={{fontSize:13,color:'#667781',margin:0,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap' as const,flex:1}}>
                      ...
                      {cv.ultima?.conteudo?.substring(0,40)||''}</p>
      ...
      {menuConversa&&(
        <>
          ...
          <div style={{position:'fixed',left:menuConversa.x,top:menuConversa.y,background:'white',borderRadius:10,boxShadow:'0 8px 32px rgba(0,0,0,0.15)',zIndex:101,minWidth:200,overflow:'hidden',padding:'4px 0'}}>
            {[
              ...
              {label:'Limpar mensagens',icon:'🗑',fn:async()=>{if(confirm('Limpar mensagens?')){await supabase.from('whatsapp_mensagens').delete().eq('conversa_id',menuConversa.id);if(ativa?.id===menuConversa.id) setMensagens([]);setMenuConversa(null)}}},
              {label:'Dados do contato',icon:'👤'},
            ].map(item=>(
              <button key={item.label} onClick={item.fn||(()=>setMenuConversa(null))} style={{display:'flex',alignItems:'center',gap:12,width:'100%',padding:'11px 16px',border:'none',background:'none',color:'#111827',fontSize:14,cursor:'pointer',textAlign:'left' as const}}
      ...
      {ativa?(
        <div style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden'}}>
                      ...
                      {m.metadata?.botoes&&m.metadata.botoes.length>0&&(
                        <div style={{display:'flex',flexDirection:'column' as const,gap:4,marginTop:6,maxWidth:'65%'}}>
                          {m.metadata.botoes.map((b:string,i:number)=>(
                            <button key={i} className="btn-resposta" onClick={()=>enviarResposta(b)}
          ...
          <div style={{background:'#f0f2f5',padding:'8px 16px',display:'flex',gap:8,alignItems:'flex-end',flexShrink:0}}>
            <button className="ibtn" style={{width:42,height:42,border:'none',background:'none',color:'#54656f',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}>
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><circle cx="12" cy="12" r="10"/><path d="M8 13s1.5 2 4 2 4-2 4-2"/><line x1="9" y1="9" x2="9.01" y2="9"/><line x1="15" y1="9" x2="15.01" y2="9"/></svg>
            </button>
            <button className="ibtn" style={{width:42,height:42,border:'none',background:'none',color:'#54656f',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}>
            ...
            {msg.trim()?(
              ...
              <button onClick={iniciarGravacao} className="ibtn" style={{width:44,height:44,border:'none',background:'none',color:'#54656f',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}>
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M12 2a3 3 0 00-3 3v7a3 3 0 006 0V5a3 3 0 00-3-3z"/><path d="M19 10v1a7 7 0 01-14 0v-1"/><line x1="12" y1="18" x2="12" y2="22"/><line x1="8" y1="22" x2="16" y2="22"/></svg>
              </button>
            )}
          </div>
        </div>
        ...
        <div style={{flex:1,display:'flex',alignItems:'center',justifyContent:'center',background:'#f8f9fa',flexDirection:'column',gap:16}}>
          ...
          <div style={{textAlign:'center'}}>
            <p style={{fontSize:32,fontWeight:300,color:'#41525d',margin:'0 0 8px',letterSpacing:'-0.5px'}}>MedIA WhatsApp</p>
