      ...
      <main style={{ flex: 1, display: 'flex', overflow: 'hidden', flexDirection: 'column' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 16, paddingTop: 14 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, flex: 1 }}>
              ...
              <div>
                ...
                <p style={{ fontSize: 10, color: config ? '#16a34a' : '#9ca3af', margin: 0 }}>● {config ? config.phone_number || config.phone_number_id : 'Desconectado'}</p>
              </div>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              {totalNaoLidas > 0 && <span style={{ fontSize: 11, fontWeight: 700, color: 'white', background: '#16a34a', padding: '2px 8px', borderRadius: 20 }}>{totalNaoLidas} novas</span>}
              {usuario && <span style={{ fontSize: 11, color: '#6b7280', background: '#f3f4f6', padding: '3px 10px', borderRadius: 20 }}>👤 {usuario.nome}</span>}
            </div>
          </div>
          <div style={{ display: 'flex', gap: 0, marginTop: 8 }}>
            <button style={tabStyle('conversas')} onClick={() => setAba('conversas')}>💬 Conversas {totalNaoLidas > 0 && <span style={{ background: '#16a34a', color: 'white', borderRadius: 10, padding: '1px 5px', fontSize: 10, marginLeft: 3 }}>{totalNaoLidas}</span>}</button>
          ...
          <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
            ...
            <div style={{ width: 300, background: 'white', borderRight: '1px solid #e5e7eb', display: 'flex', flexDirection: 'column', flexShrink: 0 }}>
              <div style={{ padding: '10px 12px', borderBottom: '1px solid #f3f4f6', display: 'flex', flexDirection: 'column', gap: 8 }}>
                <div style={{ display: 'flex', gap: 6 }}>
                  ...
                  <button onClick={() => setNovaConversa(true)} title="Nova conversa" style={{ width: 34, height: 34, borderRadius: 7, border: 'none', background: '#16a34a', color: 'white', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                ...
                <div style={{ display: 'flex', gap: 4 }}>
                  {(['todas','ia','humano'] as const).map(f => (
                    <button key={f} onClick={() => setFiltroModo(f)} style={{ flex: 1, padding: '4px 0', fontSize: 10, fontWeight: filtroModo === f ? 700 : 400, borderRadius: 5, border: '1px solid ' + (filtroModo === f ? '#16a34a' : '#e5e7eb'), background: filtroModo === f ? '#f0fdf4' : 'white', color: filtroModo === f ? '#16a34a' : '#9ca3af', cursor: 'pointer' }}>
              ...
              {novaConversa && (
                <div style={{ padding: '12px', borderBottom: '1px solid #e5e7eb', background: '#f0fdf4' }}>
                  <p style={{ fontSize: 11, fontWeight: 700, color: '#166534', margin: '0 0 8px' }}>Nova conversa</p>
                  <input value={novoTel} onChange={e => setNovoTel(e.target.value)} style={{ width: '100%', padding: '7px 10px', fontSize: 12, borderRadius: 7, border: '1px solid #bbf7d0', marginBottom: 6, outline: 'none' }} placeholder="Numero (ex: 5511999887766)"/>
                  <input value={novaMsgTexto} onChange={e => setNovaMsgTexto(e.target.value)} style={{ width: '100%', padding: '7px 10px', fontSize: 12, borderRadius: 7, border: '1px solid #bbf7d0', marginBottom: 8, outline: 'none' }} placeholder="Primeira mensagem..."/>
                  <div style={{ display: 'flex', gap: 6 }}>
                    <button onClick={iniciarConversa} disabled={iniciando || !novoTel || !novaMsgTexto} style={{ flex: 1, padding: '7px', fontSize: 11, fontWeight: 700, borderRadius: 7, border: 'none', background: '#16a34a', color: 'white', cursor: 'pointer' }}>{iniciando ? 'Enviando...' : 'Iniciar'}</button>
              ...
              <div style={{ flex: 1, overflow: 'auto' }}>
                {conversasFiltradas.length === 0 ? (
                  ...
                  <div key={c.id} onClick={() => setAtiva(c)} style={{ padding: '9px 12px', borderBottom: '1px solid #f9fafb', cursor: 'pointer', background: ativa?.id === c.id ? '#f0fdf4' : 'white', borderLeft: ativa?.id === c.id ? '3px solid #16a34a' : '3px solid transparent' }}>
                    <div style={{ display: 'flex', gap: 8 }}>
                      <div style={{ position: 'relative', flexShrink: 0 }}>
                        <div style={{ width: 34, height: 34, borderRadius: '50%', background: ativa?.id === c.id ? '#dcfce7' : '#f3f4f6', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 700, color: ativa?.id === c.id ? '#16a34a' : '#6b7280' }}>{ini(nomeCv(c))}</div>
                        <span style={{ position: 'absolute', bottom: 0, right: 0, width: 10, height: 10, borderRadius: '50%', background: c.modo === 'humano' ? '#f59e0b' : '#16a34a', border: '1.5px solid white' }}/>
                      </div>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                          ...
                          <div style={{ display: 'flex', gap: 3, marginLeft: 4, flexShrink: 0 }}>
                            {c.naoLidas > 0 && <span style={{ fontSize: 9, fontWeight: 700, color: 'white', background: '#16a34a', width: 15, height: 15, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{c.naoLidas}</span>}
            ...
            {ativa ? (
              <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: '#f0f2f5' }}>
                ...
                <div style={{ background: 'white', borderBottom: '1px solid #e5e7eb', padding: '10px 16px', display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
                  <div style={{ position: 'relative' }}>
                    <div style={{ width: 36, height: 36, borderRadius: '50%', background: '#dcfce7', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 700, color: '#16a34a' }}>{ini(nomeCv(ativa))}</div>
                    <span style={{ position: 'absolute', bottom: 0, right: 0, width: 11, height: 11, borderRadius: '50%', background: ativa.modo === 'humano' ? '#f59e0b' : '#16a34a', border: '2px solid white' }}/>
                  </div>
                  <div style={{ flex: 1 }}>
                  ...
                  <div style={{ display: 'flex', gap: 6 }}>
                    ...
                    {ativa.modo === 'ia' ? (
                      ...
                      <button onClick={devolverParaIA} style={{ fontSize: 11, fontWeight: 700, color: '#166534', background: '#f0fdf4', border: '1px solid #bbf7d0', padding: '4px 12px', borderRadius: 6, cursor: 'pointer' }}>
                        🤖 Devolver para IA
                      </button>
                    )}
                  </div>
                  ...
                  {mensagens.map(m => {
                    ...
                    return (
                      <div key={m.id} style={{ display: 'flex', justifyContent: rec ? 'flex-start' : 'flex-end' }}>
                        <div style={{ maxWidth: '68%', padding: '7px 11px', borderRadius: rec ? '4px 10px 10px 10px' : '10px 4px 10px 10px', background: rec ? 'white' : (isIA ? '#dcfce7' : '#dbeafe'), border: rec ? '1px solid #e5e7eb' : (isIA ? '1px solid #bbf7d0' : '1px solid #bfdbfe') }}>
                          {(isIA || remetente) && <p style={{ fontSize: 9, color: isIA ? '#16a34a' : '#2563eb', fontWeight: 700, margin: '0 0 2px', textTransform: 'uppercase', letterSpacing: '0.06em' }}>{isIA ? 'Sofia IA' : remetente}</p>}
              ...
              <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#f0f2f5', flexDirection: 'column', gap: 12 }}>
                ...
                <button onClick={() => setNovaConversa(true)} style={{ fontSize: 12, color: '#16a34a', background: '#f0fdf4', border: '1px solid #bbf7d0', padding: '7px 16px', borderRadius: 8, cursor: 'pointer', fontWeight: 600 }}>+ Nova conversa</button>
              </div>
            )}
          </div>
        )}
              ...
              <div style={{ background: 'white', border: '1px solid #e5e7eb', borderRadius: 14, overflow: 'hidden' }}>
                <div style={{ padding: '14px 20px', borderBottom: '1px solid #f3f4f6', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    ...
                    <button onClick={() => setSofiaAtivo(!sofiaAtivo)} style={{ width: 40, height: 22, borderRadius: 11, border: 'none', background: sofiaAtivo ? '#16a34a' : '#d1d5db', cursor: 'pointer', position: 'relative' }}>
                ...
                <div style={{ padding: '18px 20px' }}>
                  ...
                  <div style={{ background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: 9, padding: '10px 14px', margin: '14px 0' }}>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6 }}>
                      {['Responde 24h automaticamente','Agenda consultas no sistema','Envia horarios disponiveis','Reconhece pacientes cadastrados','Escalona para humano quando pedido','Segue fluxo de menu configurado'].map(c => (
                        <div key={c} style={{ display: 'flex', gap: 6 }}><span style={{ color: '#16a34a' }}>✓</span><span style={{ fontSize: 11, color: '#374151' }}>{c}</span></div>
                      ))}
                    </div>
                  </div>
                  <div style={{ display: 'flex', gap: 8 }}>
                    <button onClick={salvarSofia} disabled={salvandoSofia} style={{ padding: '9px 20px', borderRadius: 9, border: 'none', background: '#16a34a', color: 'white', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>{salvandoSofia ? 'Salvando...' : 'Salvar'}</button>
                    ...
                    {sofiaMsg && <span style={{ fontSize: 12, color: '#16a34a', fontWeight: 600, alignSelf: 'center' }}>{sofiaMsg}</span>}
                  </div>
                </div>
              </div>
            </div>
            ...
            <div style={{ maxWidth: 680 }}>
              ...
              <div style={{ background: 'white', border: '1px solid #e5e7eb', borderRadius: 14, overflow: 'hidden' }}>
                ...
                <form onSubmit={adicionarAtendente} style={{ padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: 12 }}>
                  ...
                  {atMsg && <div style={{ background: atMsg.tipo === 'ok' ? '#f0fdf4' : '#fef2f2', border: '1px solid ' + (atMsg.tipo === 'ok' ? '#bbf7d0' : '#fecaca'), borderRadius: 8, padding: '8px 12px' }}><p style={{ fontSize: 12, color: atMsg.tipo === 'ok' ? '#166534' : '#dc2626', margin: 0 }}>{atMsg.texto}</p></div>}
                  <button type="submit" disabled={salvandoAt} style={{ padding: '10px', borderRadius: 9, border: 'none', background: '#16a34a', color: 'white', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>{salvandoAt ? 'Adicionando...' : 'Adicionar atendente'}</button>
                </form>
              </div>
            </div>
          </div>
        ...
        {aba === 'configuracao' && (
          <div style={{ flex: 1, overflow: 'auto', padding: 24 }}>
            <div style={{ maxWidth: 680 }}>
              {config && (
                <div style={{ background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: 12, padding: '12px 16px', marginBottom: 18, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    ...
                    <div>
                      <p style={{ fontSize: 12, fontWeight: 700, color: '#166534', margin: 0 }}>WhatsApp conectado</p>
                      <p style={{ fontSize: 11, color: '#16a34a', margin: 0 }}>{config.phone_number} · {config.nome_exibicao}</p>
                    </div>
                  </div>
              ...
              <div style={{ background: 'white', border: '1px solid #e5e7eb', borderRadius: 14, overflow: 'hidden', marginBottom: 18 }}>
                ...
                <form onSubmit={salvarConfig} style={{ padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: 12 }}>
                  ...
                  {cfgMsg && <div style={{ background: cfgMsg.tipo === 'ok' ? '#f0fdf4' : '#fef2f2', border: '1px solid ' + (cfgMsg.tipo === 'ok' ? '#bbf7d0' : '#fecaca'), borderRadius: 8, padding: '8px 12px' }}><p style={{ fontSize: 12, color: cfgMsg.tipo === 'ok' ? '#166534' : '#dc2626', margin: 0 }}>{cfgMsg.texto}</p></div>}
                  <button type="submit" disabled={salvando} style={{ padding: '10px', borderRadius: 9, border: 'none', background: '#16a34a', color: 'white', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>{salvando ? 'Validando...' : config ? 'Atualizar' : 'Conectar WhatsApp'}</button>
                </form>
              </div>
              <div style={{ background: 'white', border: '1px solid #e5e7eb', borderRadius: 14, overflow: 'hidden' }}>
                <div style={{ padding: '14px 20px', borderBottom: '1px solid #f3f4f6' }}>
